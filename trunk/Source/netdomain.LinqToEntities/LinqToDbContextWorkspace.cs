//-------------------------------------------------------------------------------
// <copyright file="LinqToDbContextWorkspace.cs" company="bbv Software Services AG">
...
namespace netdomain.LinqToEntities
{
    ...
    /// </summary>
    public class LinqToDbContextWorkspace : IWorkspace<DbContext>, IConnectionManager
    {
        /// <summary>
        /// Holds the <see cref="DbContext"/> of the Entity framework.
        /// </summary>
        ...
        public LinqToDbContextWorkspace(DbContext context)
        {
            if (context == null)
            {
        ...
        public DbContext WrappedInstance
        {
        ...
        {
            get { return this.context.Database.Connection; }
        }
        ...
        public IDbConnection Disconnect()
        {
            if (this.IsConnected)
            {
                ...
                {
                    this.releaseConnectionDelegate = this.objectContext.CreateReleaseConnectionDelegate();
                }
        ...
        public IDbConnection Reconnect()
        {
            if (!this.IsConnected)
            {
                ...
                {
                    this.ensureConnectionDelegate = this.objectContext.CreateEnsureConnectionDelegate();
                }
        ...
        public void SubmitChanges()
        {
            ...
            foreach (var extension in this.extensions)
            {
                ...
                {
                    deletedEntities = this.objectContext.GetEntitiesFromObjectStateManager(EntityState.Deleted);
                }

                if (addedEntities == null)
                {
                    addedEntities = this.objectContext.GetEntitiesFromObjectStateManager(EntityState.Added);
                }

                if (modifiedEntities == null)
                {
                    modifiedEntities = this.objectContext.GetEntitiesFromObjectStateManager(EntityState.Modified);
                }
            ...
            {
                var exception = new OptimisticOfflineLockException(e.Message, e, e.Entries.Select(se => new ConflictedObject(se.Entity, se.Entity.GetType())).ToArray());
        ...
        {
            var key = this.objectContext.CreateEntityKey(this.objectContext.GetEntitySetName<T>(), entityWithKey);

            // Get the original item based on the entity key from the context
            // or from the database.
            return this.objectContext.GetObjectByKey(key) as T;
        }
        ...
        {
            var key = this.objectContext.CreateEntityKey(this.objectContext.GetEntitySetName<T>(), entity);
            object originalItem;
        ...
        {
            this.context.Set<T>().Attach(entityWithKey);
        ...
        {
            return this.context.Database.SqlQuery<T>(commandText, parameters);
        }
